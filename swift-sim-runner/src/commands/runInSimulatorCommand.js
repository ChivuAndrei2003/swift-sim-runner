...
function registerRunInSimulatorCommand(context, { output, state, statusBar }) {
  const disposable = vscode.commands.registerCommand('swift-sim-runner.runInSimulator', async () => {
    ...
    try {
      ...
      output.appendLine(`[error] ${error instanceof Error ? error.stack : String(error)}`);
      output.show(true);
      vscode.window.showErrorMessage('Swift Sim Runner failed. See output for details.');
    } finally {
